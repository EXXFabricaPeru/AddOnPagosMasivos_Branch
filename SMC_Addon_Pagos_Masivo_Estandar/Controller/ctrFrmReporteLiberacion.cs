...
namespace SMC_APM.Controller
{
    class ctrFrmReporteLiberacion
    {
        ...
        #region Constructor
        public ctrFrmReporteLiberacion()
        {
        }

        public ctrFrmReporteLiberacion(SAPbouiCOM.Application sboApplication, SAPbobsCOM.Company sboCompany)
        {
        ...
        public void cargarFormulario(string frmUID)
        {
            ...
            sapObj.sapCargarFormulario(sboApplication, frmUID, Properties.Resources.frmReporteRCLiberacion, "");
        ...
        //Obtiene documentos para el reporte de liberacion
        private void getdocumentos()
        {
            string consultaDB1 = "";
            try
            {
                consultaDB1 = "CALL \"SMC_APM_REPORTE_LIBERACION\" ('" + txt1.Value.ToString() + "','" + txt2.Value.ToString() + "','" + txt3.Value.ToString() + "','" + txt4.Value.ToString() + "') ";

                dta1.Rows.Clear();
                dta1.ExecuteQuery(consultaDB1);

                mtx1.Columns.Item("1").DataBind.Bind("dta_1_1", "Code");
                mtx1.Columns.Item("2").DataBind.Bind("dta_1_1", "U_SMC_RCLIB");
