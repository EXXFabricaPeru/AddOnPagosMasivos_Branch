...
BEGIN
	...
	declare retenProv varchar(1);
	
	select "MltpBrnchs" into tieneSucursales from OADM;
	select U_VALOR into retenProv from "@SMC_APM_CONFIAPM" where "Code" = '2';
	...
	then
		...
		select 
			...
			coalesce(MAX(T0.U_EXX_RETPRO),:retenProv) as "RetPro",
			(select MAX(TX0."Series") from NNM1 TX0 where ifnull(TX0."BPLId",'0') = ifnull(MAX(T0."BPLId"),'0') and coalesce(TX0.U_EXC_CR,'') != 'Y' and TX0."ObjectCode" = '46' and TX0."Locked"='N' and TX0."Indicator" = case when TX0."Indicator" = 'Valor de p' then TX0."Indicator" else :indicador end) as "CodSerPago",
			(select MAX(TX0."SeriesName") from NNM1 TX0 where ifnull(TX0."BPLId",'0') = ifnull(MAX(T0."BPLId"),'0') and coalesce(TX0.U_EXC_CR,'') != 'Y' and TX0."ObjectCode" = '46' and TX0."Locked"='N' and TX0."Indicator" = case when TX0."Indicator" = 'Valor de p' then TX0."Indicator" else :indicador end)  as "NomSerPago",
			(select MAX(TX0."Series") from NNM1 TX0 where ifnull(TX0."BPLId",'0') = ifnull(MAX(T0."BPLId"),'0') and coalesce(TX0.U_EXC_CR,'') = 'Y' and TX0."ObjectCode" = '46' and TX0."Locked"='N' and TX0."Indicator" = case when TX0."Indicator" = 'Valor de p' then TX0."Indicator" else :indicador end) as "CodSerReten",
			(select MAX(TX0."SeriesName") from NNM1 TX0 where ifnull(TX0."BPLId",'0') = ifnull(MAX(T0."BPLId"),'0') and coalesce(TX0.U_EXC_CR,'') = 'Y' and TX0."ObjectCode" = '46' and TX0."Locked"='N' and TX0."Indicator" = case when TX0."Indicator" = 'Valor de p' then TX0."Indicator" else :indicador end) as "NomSerReten"
		from OBPL T0 order by 1;
	end if;
END;