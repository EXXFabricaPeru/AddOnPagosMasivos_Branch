...
FROM (
	...
	left(replace(T2."Account",'-',''),8) ||'00'||right(replace(T2."Account",'-',''),length(replace(T2."Account",'-',''))-8) AS "004-023(20)",--CUENTA CARGO
...
LEFT(T1."U_EXP_CARDNAME"||replicate(' ',40),40) as "038-077(40)",--Nombre Beneficiario
...
rpad(coalesce(T4."NumAtCard",T5."NumAtCard",TO_VARCHAR(T6."TransId")),12,' ') AS "094-105(12)",--Número de documento
'N' as "106-106(1)",--Abono Agrupado - S=Abono Agrupado N=Abono Individual
rpad(coalesce(T4."NumAtCard",T5."NumAtCard",TO_VARCHAR(T6."TransId")),40,' ')  as "107-146(40)",--Referencia-Opcional
replicate(' ',1)  as "147-147(1)",--Indicador de aviso -opcional
replicate(' ',50)  as "148-197(50)",--Medio de aviso-opcional
rpad(left(T1."U_EXP_CARDNAME",30),30,' ')  as "198-227(30)",--Persona Contacto - opcional
