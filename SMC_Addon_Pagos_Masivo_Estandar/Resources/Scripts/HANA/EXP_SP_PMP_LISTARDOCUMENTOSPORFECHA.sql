...
begin
	...
	(
		...
		inner 	join DSC1				T3 on T3."GLAccount" 	= T2."AcctCode"
		inner  	join ORIN				T4 on T4."DocEntry" 	= rtrim(ltrim(T1."U_SMC_DOCENTRY")) and T1."U_SMC_TIPO_DOCUMENTO" = 'NC-C'
		...
		inner 	join OACT 				T2 on T2."FormatCode" 	= replace(T0."U_SMC_CTACONT",'-','')
		inner 	join DSC1				T3 on T3."GLAccount" 	= T2."AcctCode"
		inner 	join ODPO				T4 on T4."DocEntry" 	= rtrim(ltrim(T1."U_SMC_DOCENTRY")) and T1."U_SMC_TIPO_DOCUMENTO" IN ('FA-P','SA-P')
		inner 	join OCRD				T5 on T4."CardCode"		= T5."CardCode"
		...
		inner 	join OACT 				T2 on T2."FormatCode" 	= replace(T0."U_SMC_CTACONT",'-','')
		inner 	join DSC1				T3 on T3."GLAccount" 	= T2."AcctCode"
		inner 	join OPDF				T4 on T4."DocEntry" 	= rtrim(ltrim(T1."U_SMC_DOCENTRY")) and T1."U_SMC_TIPO_DOCUMENTO" = 'SP'
		inner 	join OCRD				T5 on T4."CardCode"		= T5."CardCode"
		...
		inner 	join OACT 				T2 on T2."FormatCode" 	= replace(T0."U_SMC_CTACONT",'-','')
		inner 	join DSC1				T3 on T3."GLAccount" 	= T2."AcctCode"
		inner 	join JDT1				T4 on T4."TransId" 		= rtrim(ltrim(T1."U_SMC_DOCENTRY")) and  T1."U_SMC_TIPO_DOCUMENTO" = 'AS'
		...
		select DISTINCT 		
			...
			T3."BankCode" 				as "CodBanco",
			T2."AcctCode" 				as "CodCtaBanco",
			T0."U_CTA_BANCO_ML"			as "NumCtaBanco",
			T1."U_DOCENTRY"			as "DocEntryDocumento",
			'24'						as "TipoDocumento",
			'' 							as "NroDocSUNAT",
			'O'							as "EstadoDocumento",
		...
		from "@EXD_OEPG" 			T0 
		inner 	join "@EXD_EPG1" 	T1 on T0."DocEntry" 		= T1."DocEntry"
		inner 	join OACT 				T2 on T2."FormatCode" 	= replace(T0."U_CTA_BANCO_ML",'-','')
		inner 	join DSC1				T3 on T3."GLAccount" 	= T2."AcctCode"
		inner 	join JDT1				T4 on T4."TransId" 		= ltrim(rtrim(T1."U_DOCENTRY")) and  T1."U_TIPO_DOCUMENTO" = 'PR'
		inner 	join OCRD				T5 on T5."CardCode"		= T4."ShortName"
		left 	join JDT2				T6 on T4."TransId"		= T6."AbsEntry"								 
