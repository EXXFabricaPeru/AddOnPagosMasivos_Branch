...
FROM (
	...
	--T1."U_EXP_DOCENTRYDOC"||replicate(' ',20-LENGTH(T1."U_EXP_DOCENTRYDOC")) AS "24-43 (20)",
	T3."NumAtCard"||replicate(' ',20-LENGTH(T3."NumAtCard")) AS "24-43 (20)",
	...
	RIGHT(replicate('0',15)||REPLACE(REPLACE(CAST(CAST(T1."U_EXP_IMPORTE" AS DECIMAL(18,2)) AS NVARCHAR(15)),',',''),'.',''),15)
		AS "54-68 (15)",
	CASE T1."U_EXP_CODBANCOPROV" WHEN '003' THEN ' ' else '1' end  AS "69-69 (1)",
	...
	CASE T1."U_EXP_MEDIODEPAGO" WHEN 'TB' THEN
		CASE T1."U_EXP_CODBANCOPROV"
			WHEN '003' THEN 
				CASE ifnull((SELECT max("UsrNumber2") FROM OCRB TX0 WHERE TX0."CardCode" = T2."CardCode" 
						and TX0."BankCode" = T1."U_EXP_CODBANCOPROV" and TX0."Account" = T1."U_EXP_NROCTAPROV"),'')
				WHEN 'A' THEN '002'
				WHEN 'C' THEN '001'
	...
	AND T1."U_EXP_CODCTABANCO"=:glaccount
	AND T1."U_EXP_COD_SUCURSAL"=:codSucursal
	AND T1."U_EXP_TIPODOC" IN ('24','30','46','140')
	AND IFNULL(T1."U_EXP_NROCTAPROV",'')!='' AND T1."U_EXP_MEDIODEPAGO" IN ('TB','CG')
	AND T1."U_EXP_SLC_RETENCION"='N'
	----- FIN -----
